...
class MASt3RBaseStereoViewDataset(BaseStereoViewDataset):
    ...
    def _crop_resize_if_necessary(self, image, depthmap, intrinsics, resolution, rng=None, info=None):
        ...
        cropped_image, _, _ = cropping.crop_image_depthmap(cropped_image, depthmap, intrinsics, crop_bbox)

        return image, depthmap, intrinsics2, cropped_image
    ...
    def generate_crops_from_pair(self, view1, view2, resolution, aug_crop_arg, n_crops=4, rng=np.random):
        ...
        if aug_crop_arg is False:
            ...
            for i in range(2):
                view = views[i]
                view['img'], view['depthmap'], view['camera_intrinsics'], _ = self._crop_resize_if_necessary(view['img'],
                                                                                                          view['depthmap'],
...
def transpose_to_landscape(view, revert=False):
    ...
    if width < height:
        ...
        assert view['img_orig'].shape == (height, width, 3)
        view['img_orig'] = view['img_orig'].swapaxes(0, 1)
        
        assert view['clip_emb'].shape == (3, height, width)
        view['clip_emb'] = view['clip_emb'].swapaxes(1, 2)

        view['dino_emb'] = view['dino_emb'].swapaxes(1, 2)
...
def rotate_90(views, k=1):
    ...
    for view in views:
        ...
        view['clip_emb'] = torch.rot90(view['clip_emb'], k=k, dims=(-2, -1))
        view['dino_emb'] = torch.rot90(view['dino_emb'], k=k, dims=(-2, -1))
        view['img_orig'] = np.rot90(view['img_orig'], k=k).copy()
