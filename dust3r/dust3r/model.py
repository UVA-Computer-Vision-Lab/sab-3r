...
# DUSt3R model class
# --------------------------------------------------------
import dust3r.utils.path_to_croco  # noqa: F401 # isort: skip # it is used, and has to be imported first
import os
...
from models.pos_embed import get_1d_sincos_pos_embed_from_grid
...
class FlashDUSt3R(
    ...
    def _decoder(self, encoded_feats, positions, image_ids):
        ...
        image_pos_emb = torch.from_numpy(
            get_1d_sincos_pos_embed_from_grid(self.dec_embed_dim, np.arange(num_images))
        ).float().to(x.device)
