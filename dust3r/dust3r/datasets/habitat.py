...
class Habitat(BaseStereoViewDataset):
    def __init__(self, size, *args, ROOT, **kwargs):
        ...
        with open(osp.join(self.ROOT, f'Habitat_all_scenes.txt')) as f:
            self.scenes = f.read().splitlines()
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        for view_index in two_random_views:
            ...
            for ii in range(view_index, view_index + 5):
                try:
                    image, depthmap, intrinsics, camera_pose, image_crop = self._load_one_view(data_path, key, ii % 5, resolution, rng)
            ...
            transform = T.Compose([
                T.ToTensor(),
                norm if use_norm else T.Lambda(lambda x: x)
            ])
            image_tensor = transform(image) # 3, input size[1], input size[0]
            scaling_factor = 14/16
            new_size = (int(image_tensor.shape[1] * scaling_factor), int(image_tensor.shape[2] * scaling_factor))
            resized_tensor = F.interpolate(image_tensor.unsqueeze(0), size=new_size, mode='bilinear', align_corners=False).squeeze(0)
            views.append(dict(
                img=image,
                img_orig=np.array(image_crop),
                depthmap=depthmap,
                ...
                clip_emb=image_tensor,
                dino_emb=resized_tensor))
        return views
    ...
    def _load_one_view(self, data_path, key, view_index, resolution, rng):
        ...
        image, depthmap, intrinsics, image_crop = self._crop_resize_if_necessary(
            image, depthmap, intrinsics, resolution, rng, info=impath)
        return image, depthmap, intrinsics, camera_pose, image_crop
